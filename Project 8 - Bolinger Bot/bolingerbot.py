    ...
    def start_trading(
        ...
        for date in trading_dates:
            ...
            if factor <= -1 * bolinger_factor:
                balance -= current_price * transaction_amount
                balance -= 5 * transaction_amount
                # balance -= (transaction_amount * (.005 * current_price))
            ...
            value = value.append(
                pd.DataFrame(
                    {"Value": [current_price * holding + balance]}, index=[date]
                )
            )
        return value
